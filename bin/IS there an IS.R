...
is_IS=function(read.name)
{
  ...
  #At the moment the script only works on read which have one breakpoint:
  
  if(!any(is.na(c(pre_break,post_break))))
  {
...
write.table(kolp,paste(args[3],"_kolp.txt",sep=""),col.names = F,row.names = F)
